    ...
    useCallback(
      async () => {
        ...
        try {
          ...
          console.error(
            "Failed to load user notification count:",
            requestError
          );
        }
      },
  ...
  return (
    ...
    >
      <button
        ...
        className={`relative flex h-10 w-10 items-center justify-center rounded-full border transition sm:h-11 sm:w-11 ${
          open
            ...
            : "border-black/10 bg-white text-neutral-700 hover:border-black/20 hover:bg-neutral-50"
        }`}
      >
        <Bell
      ...
      <div
        ...
        className={`absolute right-0 top-[calc(100%+12px)] z-[100] w-[calc(100vw-2rem)] max-w-[400px] origin-top-right overflow-hidden rounded-[26px] border border-black/[0.08] bg-white shadow-[0_28px_90px_rgba(15,23,42,0.18)] transition duration-200 ${
          open
